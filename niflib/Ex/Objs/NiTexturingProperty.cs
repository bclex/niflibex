    ...
    public class NiTexturingProperty : NiProperty
    {
            ...
            if (info.version >= 0x0A000100)
            {
                ...
                for (var i2 = 0; i2 < shaderTextures.Count; i2++)
                {
                    ...
                    if (shaderTextures[i2].hasMap)
                    {
                        ...
                        if (info.version <= 0x14000005)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.clampMode, s, info);
                            Nif.NifStream(out shaderTextures[i2].map.filterMode, s, info);
                        }
                        if (info.version >= 0x14010003)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.flags, s, info);
                        }
                        if (info.version >= 0x14050004)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.maxAnisotropy, s, info);
                        }
                        if (info.version <= 0x14000005)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.uvSet, s, info);
                        }
                        if (info.version <= 0x0A040001)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.ps2L, s, info);
                            Nif.NifStream(out shaderTextures[i2].map.ps2K, s, info);
                        }
                        if (info.version <= 0x0401000C)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.unknown1, s, info);
                        }
                        if (info.version >= 0x0A010000)
                        {
                            Nif.NifStream(out shaderTextures[i2].map.hasTextureTransform, s, info);
                            if (shaderTextures[i2].map.hasTextureTransform)
                            {
                                Nif.NifStream(out shaderTextures[i2].map.translation, s, info);
                                Nif.NifStream(out shaderTextures[i2].map.scale, s, info);
                                Nif.NifStream(out shaderTextures[i2].map.rotation, s, info);
                                Nif.NifStream(out shaderTextures[i2].map.transformMethod, s, info);
                                Nif.NifStream(out shaderTextures[i2].map.center, s, info);
                            }
                        }
                        Nif.NifStream(out shaderTextures[i2].mapId, s, info);
                    }
                }
            }
            ...
            if (info.version >= 0x0A000100)
            {
                ...
                for (var i2 = 0; i2 < shaderTextures.Count; i2++)
                {
                    ...
                    if (shaderTextures[i2].hasMap)
                    {
                        if (info.version <= 0x03010000)
                        {
                            WriteRef((NiObject)shaderTextures[i2].map.image, s, info, link_map, missing_link_stack);
                        }
                        if (info.version >= 0x0303000D)
                        {
                            WriteRef((NiObject)shaderTextures[i2].map.source, s, info, link_map, missing_link_stack);
                        }
                        if (info.version <= 0x14000005)
                        {
                            Nif.NifStream(shaderTextures[i2].map.clampMode, s, info);
                            Nif.NifStream(shaderTextures[i2].map.filterMode, s, info);
                        }
                        if (info.version >= 0x14010003)
                        {
                            Nif.NifStream(shaderTextures[i2].map.flags, s, info);
                        }
                        if (info.version >= 0x14050004)
                        {
                            Nif.NifStream(shaderTextures[i2].map.maxAnisotropy, s, info);
                        }
                        if (info.version <= 0x14000005)
                        {
                            Nif.NifStream(shaderTextures[i2].map.uvSet, s, info);
                        }
                        if (info.version <= 0x0A040001)
                        {
                            Nif.NifStream(shaderTextures[i2].map.ps2L, s, info);
                            Nif.NifStream(shaderTextures[i2].map.ps2K, s, info);
                        }
                        if (info.version <= 0x0401000C)
                        {
                            Nif.NifStream(shaderTextures[i2].map.unknown1, s, info);
                        }
                        if (info.version >= 0x0A010000)
                        {
                            Nif.NifStream(shaderTextures[i2].map.hasTextureTransform, s, info);
                            if (shaderTextures[i2].map.hasTextureTransform)
                            {
                                Nif.NifStream(shaderTextures[i2].map.translation, s, info);
                                Nif.NifStream(shaderTextures[i2].map.scale, s, info);
                                Nif.NifStream(shaderTextures[i2].map.rotation, s, info);
                                Nif.NifStream(shaderTextures[i2].map.transformMethod, s, info);
                                Nif.NifStream(shaderTextures[i2].map.center, s, info);
                            }
                        }
                        Nif.NifStream(shaderTextures[i2].mapId, s, info);
                    }
                }
            }
        ...
        public override string AsString(bool verbose = false)
        {
            ...
            for (var i1 = 0; i1 < shaderTextures.Count; i1++)
            {
                ...
                s.AppendLine($"    Has Map:  {shaderTextures[i1].hasMap}");
                if (shaderTextures[i1].hasMap)
                {
                    s.AppendLine($"      Image:  {shaderTextures[i1].map.image}");
                    s.AppendLine($"      Source:  {shaderTextures[i1].map.source}");
                    s.AppendLine($"      Clamp Mode:  {shaderTextures[i1].map.clampMode}");
                    s.AppendLine($"      Filter Mode:  {shaderTextures[i1].map.filterMode}");
                    s.AppendLine($"      Flags:  {shaderTextures[i1].map.flags}");
                    s.AppendLine($"      Max Anisotropy:  {shaderTextures[i1].map.maxAnisotropy}");
                    s.AppendLine($"      UV Set:  {shaderTextures[i1].map.uvSet}");
                    s.AppendLine($"      PS2 L:  {shaderTextures[i1].map.ps2L}");
                    s.AppendLine($"      PS2 K:  {shaderTextures[i1].map.ps2K}");
                    s.AppendLine($"      Unknown1:  {shaderTextures[i1].map.unknown1}");
                    s.AppendLine($"      Has Texture Transform:  {shaderTextures[i1].map.hasTextureTransform}");
                    if (shaderTextures[i1].map.hasTextureTransform)
                    {
                        s.AppendLine($"        Translation:  {shaderTextures[i1].map.translation}");
                        s.AppendLine($"        Scale:  {shaderTextures[i1].map.scale}");
                        s.AppendLine($"        Rotation:  {shaderTextures[i1].map.rotation}");
                        s.AppendLine($"        Transform Method:  {shaderTextures[i1].map.transformMethod}");
                        s.AppendLine($"        Center:  {shaderTextures[i1].map.center}");
                    }
                    s.AppendLine($"      Map ID:  {shaderTextures[i1].mapId}");
                }
            }
            return s.ToString();
        ...
        internal override List<NiObject> GetRefs()
        {
            ...
            for (var i1 = 0; i1 < shaderTextures.Count; i1++)
            {
                ...
                if (shaderTextures[i1].map.source != null)
                    refs.Add((NiObject)shaderTextures[i1].map.source);
            }
            return refs;
        }
        ...
        //--BEGIN:FILE FOOT--//
        /*!
         * Gets or sets the number of texture slots defined by this texturing propery.  Known valid values are 7 and 8.
         * \param new_count The new size of the texture slot array.
         * \sa NiTexturingProperty::GetTextureCount
         */
        public int TextureCount
        {
            get => (int)textureCount;
            set => textureCount = (uint)value;
        }

        /*! 
         * Gets or sets the number of extra texture slots defined by this texturing propery.  Often zero.
         * \param new_count The new size of the extra texture slot array.
         * \sa NiTexturingProperty::GetExtraTextureCount
         */
        public int ShaderTextureCount
        {
            get => shaderTextures.Count;
            set => shaderTextures.Resize(value);
        }

        /*! 
         * Gets or sets the current apply mode for this texturing propery.  This enum value affects the way the textures will be drawn.
         ...
         */
        public TexDesc GetTexture(int n)
        {
            ...
            switch ((TexType)n)
            {
                case TexType.BASE_MAP: return baseTexture;
                case TexType.DARK_MAP: return darkTexture;
                case TexType.DETAIL_MAP: return detailTexture;
                case TexType.GLOSS_MAP: return glossTexture;
                case TexType.GLOW_MAP: return glowTexture;
                case TexType.BUMP_MAP: return bumpMapTexture;
                case TexType.NORMAL_MAP: return normalTexture;
                case TexType.PARALLAX_MAP: return parallaxTexture;
                case TexType.DECAL_0_MAP: return decal0Texture;
                case TexType.DECAL_1_MAP: return decal1Texture;
                case TexType.DECAL_2_MAP: return decal2Texture;
                case TexType.DECAL_3_MAP: return decal3Texture;
                //If we get here, an invalid texture was requested.
                default: throw new Exception("Invalid texture requested.");
            }
        }
         ...
         */
        public bool HasTexture(int n)
        {
            switch ((TexType)n)
            {
                case TexType.BASE_MAP: return hasBaseTexture;
                case TexType.DARK_MAP: return hasDarkTexture;
                case TexType.DETAIL_MAP: return hasDetailTexture;
                case TexType.GLOSS_MAP: return hasGlossTexture;
                case TexType.GLOW_MAP: return hasGlowTexture;
                case TexType.BUMP_MAP: return hasBumpMapTexture;
                case TexType.NORMAL_MAP: return hasNormalTexture;
                case TexType.PARALLAX_MAP: return hasParallaxTexture;
                case TexType.DECAL_0_MAP: return hasDecal0Texture;
                case TexType.DECAL_1_MAP: return hasDecal1Texture;
                case TexType.DECAL_2_MAP: return hasDecal2Texture;
                case TexType.DECAL_3_MAP: return hasDecal3Texture;
                default: return false;
                    //default: throw new Exception("You have specified an invalid texture type.");
            }
        }
         ...
         */
        public void ClearTexture(int n)
        {
            switch ((TexType)n)
            {
                case TexType.BASE_MAP:
                    hasBaseTexture = false;
                    baseTexture.source = null;
                    break;
                case TexType.DARK_MAP:
                    hasDarkTexture = false;
                    darkTexture.source = null;
                    break;
                case TexType.DETAIL_MAP:
                    hasDetailTexture = false;
                    detailTexture.source = null;
                    break;
                case TexType.GLOSS_MAP:
                    hasGlossTexture = false;
                    glossTexture.source = null;
                    break;
                case TexType.GLOW_MAP:
                    hasGlowTexture = false;
                    glowTexture.source = null;
                    break;
                case TexType.BUMP_MAP:
                    hasBumpMapTexture = false;
                    bumpMapTexture.source = null;
                    break;
                case TexType.NORMAL_MAP:
                    hasNormalTexture = false;
                    normalTexture.source = null;
                    break;
                case TexType.PARALLAX_MAP:
                    hasParallaxTexture = false;
                    parallaxTexture.source = null;
                    break;
                case TexType.DECAL_0_MAP:
                    hasDecal0Texture = false;
                    decal0Texture.source = null;
                    break;
                case TexType.DECAL_1_MAP:
                    hasDecal1Texture = false;
                    decal1Texture.source = null;
                    break;
                case TexType.DECAL_2_MAP:
                    hasDecal2Texture = false;
                    decal2Texture.source = null;
                    break;
                case TexType.DECAL_3_MAP:
                    hasDecal3Texture = false;
         ...
         */
        public void SetTexture(int n, TexDesc new_val)
        {
            //Make sure index is not out of range
            if (n < 0 || n > textureCount)
                throw new Exception("SetTexture - Index out of range.  Call SetTextureCount to resize.");
            //TODO:  Create a built in type for this array of textures so it can be a real array again?
            //Copy the values to the right texture
            switch ((TexType)n)
            {
         ...
         */
        public TexDesc GetShaderTexture(int n) => shaderTextures[n].textureData;
         ...
         * \sa NiTexturingProperty::GetTexture, TexType
         */
        public void SetShaderTexture(int n, TexDesc new_val)
        {
            //Make sure index is not out of range
            if (n < 0 || n > shaderTextures.Count)
                throw new Exception("SetShaderTexture - Index out of range.  Call SetShaderTextureCount to resize.");
            //Copy the values
            shaderTextures[n].textureData = new_val;
        }

        /*! 
         * Gets or sets the bump map luma offset.  This is only relevant if a texture is defined in the BUMP_MAP texture slot.  The function of this is unknown.
         * \param new_val The new bump map luma offset.
         * \sa NiTexturingProperty::GetLumaOffset
         */
        public float LumaOffset
        {
            get => bumpMapLumaOffset;
            set => bumpMapLumaOffset = value;
        }

        /*! 
         * Gets or sets the bump map luma scale.  This is only relevant if a texture is defined in the BUMP_MAP texture slot.  The function of this is unknown.
         * \param new_val The new bump map luma scale.
         * \sa NiTexturingProperty::GetLumaScale
         */
        public float LumaScale
        {
            get => bumpMapLumaScale;
            set => bumpMapLumaScale = value;
        }

        /*! 
         * Gets or sets the bump map matrix.  This is only relevant if a texture is defined in the BUMP_MAP texture slot.  The function of this is unknown.
