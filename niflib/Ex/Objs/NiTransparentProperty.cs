...
public class NiTransparentProperty : NiProperty {
	 ...
	 */
	public static NiObject Create() => new NiTransparentProperty();
	...
	internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
		...
		for (var i2 = 0; i2 < 6; i2++) {
			Nif.NifStream(out unknown[i2], s, info);
		}
	...
	internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info) {
		...
		for (var i2 = 0; i2 < 6; i2++) {
			Nif.NifStream(unknown[i2], s, info);
		}
	...
	public override string asString(bool verbose = false) {
		...
		for (var i2 = 0; i2 < 6; i2++) {
			...
			s.AppendLine($"      Unknown[{i2}]:  {unknown[i2]}");
			array_output_count++;
		}
		return s.ToString();
