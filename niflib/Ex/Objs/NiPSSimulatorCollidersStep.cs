...
public class NiPSSimulatorCollidersStep : NiPSSimulatorStep {
	...
	/*!  */
	public uint numColliders;
	/*! The colliders affecting the particle system. */
	public NiPSCollider[] colliders;
...
internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
	...
	for (var i1 = 0; i1 < colliders.Length; i1++) {
...
internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info) {
	...
	numColliders = (uint)colliders.Length;
	Nif.NifStream(numColliders, s, info);
	for (var i1 = 0; i1 < colliders.Length; i1++) {
		WriteRef((NiObject)colliders[i1], s, info, link_map, missing_link_stack);
	}
...
public override string asString(bool verbose = false) {
	...
	for (var i1 = 0; i1 < colliders.Length; i1++) {
		...
		s.AppendLine($"    Colliders[{i1}]:  {colliders[i1]}");
		array_output_count++;
	}
	return s.ToString();
...
internal override List<NiObject> GetRefs() {
	...
	for (var i1 = 0; i1 < colliders.Length; i1++) {
		if (colliders[i1] != null)
			refs.Add((NiObject)colliders[i1]);
	}
	return refs;
}
