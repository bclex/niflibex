 ...
 */
public static NiObject Create() => new NiPSysMeshUpdateModifier();
...
internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
	...
	Nif.NifStream(out numMeshes, s, info);
	meshes = new Ref[numMeshes];
...
internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info) {
	...
	Nif.NifStream(numMeshes, s, info);
	for (var i1 = 0; i1 < meshes.Length; i1++) {
		WriteRef((NiObject)meshes[i1], s, info, link_map, missing_link_stack);
	}
...
public override string asString(bool verbose = false) {
	...
	numMeshes = (uint)meshes.Length;
	s.AppendLine($"  Num Meshes:  {numMeshes}");
	...
	for (var i1 = 0; i1 < meshes.Length; i1++) {
		...
		s.AppendLine($"    Meshes[{i1}]:  {meshes[i1]}");
		array_output_count++;
	}
	return s.ToString();
