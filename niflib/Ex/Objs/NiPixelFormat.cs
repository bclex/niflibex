...
internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
	...
	if (info.version <= 0x0A030002) {
		...
		for (var i2 = 0; i2 < 8; i2++) {
			Nif.NifStream(out oldFastCompare[i2], s, info);
		}
	}
	...
	if (info.version >= 0x0A030003) {
		for (var i2 = 0; i2 < 4; i2++) {
			Nif.NifStream(out channels[i2].type, s, info);
			Nif.NifStream(out channels[i2].convention, s, info);
			Nif.NifStream(out channels[i2].bitsPerChannel, s, info);
			Nif.NifStream(out channels[i2].isSigned, s, info);
		}
	}
...
public override string AsString(bool verbose = false) {
	...
	for (var i1 = 0; i1 < 8; i1++) {
		...
		s.AppendLine($"    Old Fast Compare[{i1}]:  {oldFastCompare[i1]}");
		array_output_count++;
	}
	...
	for (var i1 = 0; i1 < 4; i1++) {
		...
		s.AppendLine($"    Type:  {channels[i1].type}");
		s.AppendLine($"    Convention:  {channels[i1].convention}");
		s.AppendLine($"    Bits Per Channel:  {channels[i1].bitsPerChannel}");
		s.AppendLine($"    Is Signed:  {channels[i1].isSigned}");
	}
	return s.ToString();
