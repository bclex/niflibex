...
//--BEGIN:FILE FOOT--//
         ...
         */
        public void RemoveChild(NiAVObject obj)
        {
            //Search child list for the one to remove
            foreach (var it in children)
                if (it == obj) it.Parent = null;
            children.Remove(obj);
        }
         ...
         */
        public IList<NiAVObject> Children => children;
         ...
         */
        public IList<NiDynamicEffect> Effects => effects;
        ...
        public bool IsSplitMeshProxy
        {
            ...
            get
            {
                ...
                for (var i = 0; i < children.Count; ++i)
                {
                    ...
                    if (!children[i].Visibility) return false;
                }
        ...
        */
        public void GoToSkeletonBindPosition()
        {
            ...
            foreach (var it in skins)
            {
                ...
                if (bone_nodes.Count != skin_data.BoneCount)
