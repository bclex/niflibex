...
internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
	...
	if (info.userVersion2 == 130) {
		if ((dataSize > 0)) {
			...
			for (var i3 = 0; i3 < segment.Length; i3++) {
				...
				if (info.userVersion2 == 130) {
					...
					for (var i5 = 0; i5 < segment[i3].subSegment.Length; i5++) {
		...
		if (((numSegments < totalSegments) && (dataSize > 0))) {
			...
			for (var i3 = 0; i3 < segmentData.segmentStarts.Length; i3++) {
			...
			for (var i3 = 0; i3 < segmentData.perSegmentData.Length; i3++) {
				...
				for (var i4 = 0; i4 < segmentData.perSegmentData[i3].cutOffsets.Length; i4++) {
			...
			for (var i3 = 0; i3 < segmentData.ssfFile.Length; i3++) {
	...
	if (info.userVersion2 == 100) {
		...
		for (var i2 = 0; i2 < segment.Length; i2++) {
			...
			if (info.userVersion2 == 130) {
				...
				for (var i4 = 0; i4 < segment[i2].subSegment.Length; i4++) {
...
internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info) {
	...
	numSegments = (uint)segment.Length;
	if (info.userVersion2 == 130) {
		if ((dataSize > 0)) {
			...
			for (var i3 = 0; i3 < segment.Length; i3++) {
				segment[i3].numSubSegments = (uint)segment[i3].subSegment.Length;
				if ((info.userVersion2 < 130)) {
		...
		if (((numSegments < totalSegments) && (dataSize > 0))) {
			segmentData.ssfLength = (ushort)segmentData.ssfFile.Length;
			segmentData.totalSegments = (uint)segmentData.perSegmentData.Length;
			segmentData.numSegments = (uint)segmentData.segmentStarts.Length;
			...
			for (var i3 = 0; i3 < segmentData.perSegmentData.Length; i3++) {
				segmentData.perSegmentData[i3].numCutOffsets = (uint)segmentData.perSegmentData[i3].cutOffsets.Length;
	...
	if (info.userVersion2 == 100) {
		...
		for (var i2 = 0; i2 < segment.Length; i2++) {
			segment[i2].numSubSegments = (uint)segment[i2].subSegment.Length;
			if ((info.userVersion2 < 130)) {
...
public override string asString(bool verbose = false) {
	...
	if ((dataSize > 0)) {
		...
		for (var i2 = 0; i2 < segment.Length; i2++) {
			...
			array_output_count = 0;
			for (var i3 = 0; i3 < segment[i2].subSegment.Length; i3++) {
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP)) {
					s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
					break;
				}
	...
	if (((numSegments < totalSegments) && (dataSize > 0))) {
		...
		array_output_count = 0;
		for (var i2 = 0; i2 < segmentData.segmentStarts.Length; i2++) {
			if (!verbose && (array_output_count > Nif.MAXARRAYDUMP)) {
				s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
				break;
			}
		...
		array_output_count = 0;
		for (var i2 = 0; i2 < segmentData.perSegmentData.Length; i2++) {
			if (!verbose && (array_output_count > Nif.MAXARRAYDUMP)) {
				s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
				break;
			}
			segmentData.perSegmentData[i2].numCutOffsets = (uint)segmentData.perSegmentData[i2].cutOffsets.Length;
			...
			array_output_count = 0;
			for (var i3 = 0; i3 < segmentData.perSegmentData[i2].cutOffsets.Length; i3++) {
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP)) {
					s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
					break;
				}
		...
		array_output_count = 0;
		for (var i2 = 0; i2 < segmentData.ssfFile.Length; i2++) {
			if (!verbose && (array_output_count > Nif.MAXARRAYDUMP)) {
				s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
				break;
			}
