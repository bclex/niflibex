...
public class NiMaterialColorController : NiPoint3InterpController {
	...
	public MaterialColor targetColor;
	/*!  */
 ...
 */
public static NiObject Create() => new NiMaterialColorController();
...
internal override void Read(IStream s, List<uint> link_stack, NifInfo info) {
	...
	if (info.version >= 0x0A010000) {
		Nif.NifStream(out targetColor, s, info);
	}
...
internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info) {
	...
	if (info.version >= 0x0A010000) {
		Nif.NifStream(targetColor, s, info);
	}
	if (info.version <= 0x0A010067) {
...
public override string asString(bool verbose = false) {
	...
	s.AppendLine($"  Target Color:  {targetColor}");
